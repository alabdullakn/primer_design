...
class PrimerHit:
    kind: str                      # "FWD" or "REV"
    exon_name: str                 # "Exon1", "Exon2", "Exon3", "Template", "Junction"
    start_0based: int
    length: int
    ...
    gc_pct: float
    score: float
    template_seq_5to3: Optional[str] = None  # only for REV: binding site on template (as given)
...
# =========================================================
# Exon design (2 forward + 1 reverse)
# =========================================================
...
def design_exon_primers(
    ...
    rev3 = best_reverse_avoiding_fwds(
        exon3_for_reverse, fwd1, fwd2, min_len, max_len, tm_target, tm_tol, dimer_k=dimer_k
    )
...
def design_qpcr_junction_primers(
    ...
    if best_pair is None:
        raise RuntimeError("No qPCR junction primer pair found. Try wider Tm tolerance or length range.")
...
def design_basic_pcr_primers(
    ...
    if best is None:
        raise RuntimeError(
            "No primer pair found. Try widening amplicon range, widening Tm tolerance, or reducing dimer_k."
        )

...
def render():
    ...
    st.markdown(
        "Paste your spliced sequence and mark the exon-exon junction with `^`.\n"
        "Example: `...ACCTG^GTTCA...`"
    )
    ...
    with c3:
        dimer_k = st.slider("3' complementarity check (k)", 3, 8, 4, 1)
    ...
    with g1:
        gc_min = st.slider("GC min (%)", 20.0, 80.0, 40.0, 1.0)
    with g2:
        gc_max = st.slider("GC max (%)", 20.0, 80.0, 60.0, 1.0)
    with g3:
    ...
    if st.button("Design qPCR primers"):
        try:
            ...
            st.write(f"Tm: {fwd.tm_c:.1f} °C | GC: {fwd.gc_pct:.1f}% | Start: {fwd.start_0based} | Length: {fwd.length}")

            st.markdown("### Reverse primer (5'->3')")
            st.code(rev.seq_5to3)
            st.write(f"Tm: {rev.tm_c:.1f} °C | GC: {rev.gc_pct:.1f}% | Start: {rev.start_0based} | Length: {rev.length}")

            if probe is not None:
                st.markdown("### TaqMan probe (5'->3')")
                st.code(probe.seq_5to3)
                st.write(f"Tm: {probe.tm_c:.1f} °C | GC: {probe.gc_pct:.1f}% | Start: {probe.start_0based} | Length: {probe.length}")

        except Exception as e:
            st.error(f"qPCR design failed: {e}")
            st.info("Tip: try widening Tm tolerance, widening GC range, or increasing amplicon max.")
